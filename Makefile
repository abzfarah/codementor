...
GO_LINKER_FLAGS ?= -ldflags \
				   "-X github.com/nomadsingles/platform/model.BuildNumber=$(BUILD_NUMBER)\
				    -X 'github.com/nomadsingles/platform/model.BuildDate=$(BUILD_DATE)'\
				    -X github.com/nomadsingles/platform/model.BuildHash=$(BUILD_HASH)\
				    -X github.com/nomadsingles/platform/model.BuildHashEnterprise=$(BUILD_HASH_ENTERPRISE)\
				    -X github.com/nomadsingles/platform/model.BuildEnterpriseReady=$(BUILD_ENTERPRISE_READY)"
...
test-postgres: start-docker prepare-enterprise
	...
	$(GO) test $(GOFLAGS) -run=$(TESTS) -test.v -test.timeout=2000s -covermode=count -coverprofile=cprofile.out -coverpkg=$(ALL_PACKAGES_COMMA) github.com/nomadsingles/platform/store || exit 1; \
	if [ -f cprofile.out ]; then \
		tail -n +2 cprofile.out >> cover.out; \
		rm cprofile.out; \
	fi; \
...
ifeq ($(BUILD_ENTERPRISE_READY),true)f
